...
CSV_DIR = config.CAIRO_CSV + "cairo_cleared/"

csv_files =  [ file for file in os.listdir(CSV_DIR) if isfile(join(CSV_DIR,file)) and '.csv' in file ]
sql_filename = "cairo_cleared.db"


def csv_to_sql(append=True) :
...
if __name__ == "__main__" :
	...
	csv_to_sql(True) # if false and the file already exists it will erase it.
